...
本页说明首次配置的关键决策。全部 197 个字段的默认值、作用和联动规则见
[后端配置与环境变量](后端环境变量.md)。
...
| 聊天模型 | `OPENAI_BASE_URL`, `OPENAI_API_KEY`, `CHAT_MODEL`, `CHAT_API_PROTOCOL` | OpenAI 兼容 Chat Completions 或 Responses 接口 |
...
| 角色 | 必需 | 用途 | 主要配置 |
|---|---|---|---|
| 主聊天模型 | 是 | 生成最终回复 | `OPENAI_*`, `CHAT_MODEL`, `CHAT_API_PROTOCOL` |
...
把主聊天、Embedding，以及所有已启用的可选模型都指向 Ollama、vLLM 等本地兼容服务。可选模型包括 `LABEL_*`、`LIFE_*`、`VISION_*`、`RESPONSE_POLICY_*`、`QUERY_REWRITE_*`、`RERANK_*`、`CROSS_RERANK_*`、`ADAPTIVE_CHUNK_*` 和 `SCHEDULE_*`；不使用的功能应保持关闭。同时关闭：
