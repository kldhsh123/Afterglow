...
| `douyin_chat_export` | Douyin Chat Export | 抖音 ChatLab JSON/JSONL | `generator=douyin-chat-export` 且 `platform=douyin` |
...
class ExamplePlugin:
    ...
    def parse(
        ...
        for idx, raw in enumerate(payload.get("messages") or []):
            ...
            if sender_uid in settings.all_self_uids:
                role: SenderRole = "self"
            elif sender_uid in settings.all_friend_uids:
                role = "friend"
            else:
                role = "other"
            messages.append(
...
uv run python -m xuwen.ingestion.cli import export.jsonl --plugin douyin_chat_export
...
[Afterglow Chat v1 格式](Afterglow专用导入格式.md)。本页只保留 ingestion plugin 的开发约定。
...
根目录的 `VERSION` 是版本号的唯一人工来源。不要分别手改 Python 项目、锁文件或
Docker 示例；在仓库根目录运行：

```bash
# 指定版本
uv run --project backend python scripts/set_version.py 0.4.0

# 或按语义版本递增
uv run --project backend python scripts/set_version.py --bump patch
uv run --project backend python scripts/set_version.py --bump minor
uv run --project backend python scripts/set_version.py --bump major
```

脚本会调用 `uv version` 同步 `backend/pyproject.toml` 与 `backend/uv.lock`，并更新
`.env.docker.example`。只检查、不修改文件时运行：

```bash
uv run --project backend python scripts/set_version.py --check
```

版本号变更由仓库维护者在发布时执行，贡献者不需要也不应该在普通 PR 中 bump 版本。
PR 工作流仍会执行只读一致性检查，用于阻止意外修改或只改其中一处的版本漂移。
