...
浏览器打开链接，粘 token，跟着 7 步走完：

1. **身份信息** — 上传 QQ / 微信导出 JSON 自动识别 UID，不必手动找 `u_xxx` / `wxid_xxx`
...
6. **导入聊天记录** — 文件直传后端，进度+耗时+断点续传，自动生成 `persona_card.md` 和作息画像
7. **访问密码** — 自动生成 `XUWEN_API_KEY`，写入 `.env`
