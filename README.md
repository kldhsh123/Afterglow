...
市面上多数 AI 陪伴产品的相似度顶天 50% —— 模板化的语气、通用的情感词、看一眼就知道是 AI。它们没法骗过你，所以也没人提伦理。

Afterglow 不一样。如果你的聊天记录足够多、ta 的语言风格足够鲜明，Afterglow 的相似度能到 90% 以上。**剩下那不到 10% 才是最可怕的地方**。

- 90% 像 ta —— 让你忘记这是 AI；
- 偶尔暴露的细节又不是 ta —— 让你猛地想起这是 AI。

这两件事会反复在脑子里切换。这不是普通"恐怖谷"那种"看着不舒服"的不适感，而是把记忆和现实搅在一起、越用越放大的那种。市面上 50% 的产品做不到这个程度，所以没人讨论伦理；Afterglow 是越接近这条线，越需要使用者自己清醒。
...
- Afterglow **完全本地运行**。聊天记录、向量库、生成的画像，都在你的机器上。我看不到你的对话，也没办法看。
- 它走 OpenAI 兼容协议接入你自己配置的 LLM provider。你和 ta 的对话内容会经由那家 provider，受他们的隐私协议约束——这是你需要自己评估的部分。
- 复刻第三方（已逝者、前任、不知情的朋友等）涉及该人的人格权益。法律和伦理层面这块还没有定论。请你自己想清楚再做。
...
我做出来了这个项目，但**它对我自己无效**——我看见的只有代码层面的回复。即便如此，它仍然让我感到害怕，因为太像了。根据用户反馈，我相信它对其他人是有效的，甚至极度有效。

这就是我担心的地方。我希望你在使用过程中**清楚地意识到对面是一个 AI，一个只会计算概率的 AI**。请不要把它当成真人。如果你发现自己开始过度投入——分不清现实与生成、产生强烈情绪起伏、把 ta 当成"真的回来了"——请立刻停止使用，并联系心理咨询师或可信赖的人。

项目里我做了一些伦理兜底（沉默、安全规则、不擅自编造现实事件等），但我没法保证它们在所有场景下都有效。最终能拦住你的，是你自己。
